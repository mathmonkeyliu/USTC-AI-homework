...
\toprule
\normalfont\textbf{图例标签（基础 CNN）} & \normalfont\textbf{图例标签（深层 CNN）} & \textbf{表~\ref{tab:results} 编号} & \textbf{超参数} \\
\midrule
...
在完成 MNIST 测试集上的模型对比后，本文进一步使用自行书写并拍摄的数字图像对训练好的模型进行推理测试。
该实验用于检验模型在非标准 MNIST 图像上的泛化表现，同时验证训练模型在实际输入场景中的可用性。
自定义测试图像存放于 \texttt{numbersbyHAND/} 目录下，文件名为 \texttt{0.png} 至 \texttt{9.png}，
分别对应人工书写的数字 0--9。

\subsection{模型选择与预测方法}

根据训练日志 \path{logs/config_*.log} 中记录的最佳测试准确率，深层 CNN 在
\path{config_1_model_2} 和 \path{config_5_model_2} 两组配置下均取得了
最高测试准确率 $99.57\%$。本文选取 \path{config_5_model_2.yaml} 对应的检查点
\path{model/mnist_cnn_config_5_model_2.pth} 作为自定义图像识别模型。
预测脚本首先根据配置文件构建相同结构的 \texttt{MNISTCNN2} 网络，
随后加载保存的 \texttt{model\_state}，并对每张输入图像输出预测类别及 softmax 置信度。

由于自定义图像来自手机或截图拍摄，其分布与 MNIST 数据集存在明显差异：
MNIST 图像为黑色背景上的白色数字，而自定义图像通常包含灰色纸面背景、光照变化和较大的空白边缘。
若仅将原图直接缩放至 $28\times28$，背景信息会被一并压缩进输入张量，
导致模型容易将多个数字误判为形状更复杂的类别（如数字 8）。
因此，预测阶段采用了与 MNIST 形式更一致的预处理流程：

\begin{enumerate}
    \item 将输入图像转换为灰度图，并通过自动对比度增强扩大前景与背景的灰度差异；
    \item 对深色笔迹图像进行反色，使数字笔迹转化为亮色前景；
    \item 使用阈值二值化去除灰色背景，仅保留主要笔迹区域；
    \item 根据非零像素的包围盒裁剪数字主体，并按比例缩放至 $20\times20$；
    \item 将缩放后的数字居中粘贴到 $28\times28$ 的黑色画布上；
    \item 使用 MNIST 数据集的均值 $0.1307$ 和标准差 $0.3081$ 进行归一化。
\end{enumerate}

这一处理流程保持了 MNIST 数据集中“居中、黑底、亮色数字”的输入形式，
从而降低了训练分布与实际预测图像分布之间的差异。
此外，预测脚本中的模型检查点参数设置为可选参数；
当命令行未显式指定检查点时，脚本会读取配置文件中 \texttt{predict.checkpoint} 指定的最优模型路径，
避免配置文件与实际加载模型不一致的问题。

\subsection{识别结果}

使用修正后的预处理流程，对 10 张自定义手写数字图像进行预测，运行命令如下：

\begin{verbatim}
python predict_handwritten.py --config config/config_5_model_2.yaml
\end{verbatim}

识别结果如表~\ref{tab:handwritten_results} 所示。
...
\begin{table}[H]
    ...
    \begin{tabular}{cccc}
        ...
        图像文件 & 真实类别 & 预测类别 & 置信度 \\
        \midrule
        \texttt{0.png} & 0 & 0 & 1.0000 \\
        \texttt{1.png} & 1 & 1 & 1.0000 \\
        \texttt{2.png} & 2 & 2 & 0.9999 \\
        \texttt{3.png} & 3 & 3 & 1.0000 \\
        \texttt{4.png} & 4 & 4 & 0.9997 \\
        \texttt{5.png} & 5 & 5 & 0.9999 \\
        \texttt{6.png} & 6 & 6 & 0.9999 \\
        \texttt{7.png} & 7 & 7 & 0.8076 \\
        \texttt{8.png} & 8 & 8 & 1.0000 \\
        \texttt{9.png} & 9 & 9 & 0.8766 \\
        \bottomrule
    \end{tabular}
\end{table}

结果表明，经过针对实际图像分布的预处理后，模型能够正确识别全部 10 张自定义手写数字图像。
其中数字 7 和数字 9 的置信度相对较低，说明这两类样本的书写形态与训练集中的典型样本仍存在一定差异；
但总体而言，深层 CNN 在经过适当输入规范化后可以较好地迁移到简单的真实手写图像识别场景。
该实验也说明，对于图像分类模型而言，推理阶段的输入预处理与训练数据分布的一致性
是影响实际识别效果的重要因素。
